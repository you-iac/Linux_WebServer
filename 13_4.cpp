...
/*子进程需要设置自己的信号处理函数*/
    ...
    while(!stop_child)
    {
        ...
        for(int i=0;i<number;i++)
        {
            ...
            本 进程负责的客户连接有数据到达/*本子进程负责的客户连接有数据到达*/
...
int main(int argc,char*argv[])
{
    ...
    addfd(epollfd,sig_pipefd[0]);addsig(SIGCHLD,sig_handler);
...
/*保存第user_count个客户连接的相关数据*/users[user_count].address=client_address;
