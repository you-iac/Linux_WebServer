...
void lt(epoll_event*events,int number,int epollfd,int listenfd)
{
    ...
    for(int i=0;i<number;i++)
    {
        ...
        {
            //读缓存中 有未读出的数据 这段代 就被触发/*只要socket读缓存中还有未读出的数据，这段代码就被触发*/
...
void et(epoll_event*events,int number,int epollfd,int listenfd){
    ...
    {
        int sockfd=events[i].data.fd;
        if(sockfd==listenfd)/*这说明是一条连接sock到达事件*/{
            struct sockaddr_in client_address;
            socklen_t client_addrlength=sizeof(client_address);
            int connfd=accept(listenfd,(struct sockaddr*)&client_address,&client_addrlength);
            addfd(epollfd,connfd,true);/*对connfd开启ET模式*/
        }else if(events[i].events&EPOLLIN){/*这段代码不会被重复触发，所以我们循环读取数据，以确保把socket读缓存中的所有数据读出*/
            printf("event trigger once\n");
            while(1){
                memset(buf,'\0',BUFFER_SIZE);
                int ret=recv(sockfd,buf,BUFFER_SIZE-1,0);
