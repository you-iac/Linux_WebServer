...
#include<string.h>
#include<fcntl.h>
#include<stdlib.h>#include<sys/epoll.h>
#include<pthread.h>
...
void addfd(int epollfd,int fd,bool oneshot)
{
    epoll_event event;
    event.data.fd=fd;
    event.events=EPOLLIN|EPOLLET;   /*类型为EPOLLIN（可读事件）和EPOLLET（边缘触发模式）*/
    if(oneshot)
    {
...
void*worker(void*arg)
{
    ...
    /*循环读取sockfd上的数据，直到遇到EAGAIN错误*/while(1)
    {
